...
public class TradingService {
    ...
    @Transactional
    public Transaction executeTrade(Long userId, TradeRequest request) {
        ...
        if ("BUY".equalsIgnoreCase(request.getType())) {
            return executeBuy(user, request, totalAmount);
        } else if ("SELL".equalsIgnoreCase(request.getType())) {
            return executeSell(user, request, totalAmount);
        } else {
    ...
    private Transaction executeBuy(User user, TradeRequest request, double totalAmount) {
        ...
        // Update portfolio
        Optional<Portfolio> existing = portfolioRepository.findByUserIdAndSymbol(userId(user), request.getSymbol());
        if (existing.isPresent()) {
            Portfolio portfolio = existing.get();
            double newAvgPrice = ((portfolio.getAveragePrice() * portfolio.getQuantity()) + totalAmount)
    ...
    private Transaction executeSell(User user, TradeRequest request, double totalAmount) {
        Portfolio portfolio = portfolioRepository.findByUserIdAndSymbol(userId(user), request.getSymbol())
    ...
    private Transaction saveTransaction(User user, TradeRequest request, double totalAmount, Transaction.TransactionType type) {
